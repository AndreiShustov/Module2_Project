# Дополнительное практическое задание по модулю 2 "Слишком древний шифр"
...
# К вашему счастью рядом находился папирус, где были написаны правила для решения этого "ребуса":
...
def calculate_the_password(number):
    password_str = ''
    for i in range(1, number):
        for j in range(i + 1, number):
            if number % (i + j) == 0:
                password_str += str(i) + str(j)
    return password_str


for number in range(3, 21):
    print(f'Password for number {number} is:', calculate_the_password(number))
